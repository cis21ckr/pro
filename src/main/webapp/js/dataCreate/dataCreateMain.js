...
(function(W,D){
	...
	$dataCreateMain.ui = {
			...
			shpFileCheck : function(obj){
				...
				}else if(fileId == "mulitPartFilePrj"){
					$("#file_path_4").val(name);
				}
			...
			fileRead : function(obj){
				...
				if(data_type =="TEXT"){
					url = '/api/data/readText';
				}else if(data_type == "EXCEL"){
					url = '/api/data/readExcel';
				}else{
					url = '/api/data/readShp';
					//return;
				}
				console.log("data_type = " + data_type);
				...
				var after = function(res){
					...
					$("#file_path").val($dataCreateMain.ui.file_nm);
						if(data_type == "TEXT"){							
							...
							var shapeType = res.shapeInfo.shapeType.name;
			...
			xySetting : function(){
				...
				html +=optionHtml;
				html +="</select>";
				html +=$dataCreateMain.ui.geomTypeOption();
				html +="</span>";
				html +="</div>";
			 ...
			 * 
			 * @name         : geomTypeOption
			 * @description  : geomType에 대한 option을 리턴 
			 * @date         : 2018. 07. 13. 
			 * @author	     : 최재영
			 * @history 	 :
			 */
			geomTypeOption : function(){
				var html="<span>geom 타입";
				html += "<select id='geomType'>";
				html +="<option value='UTM-K'>UTM-K</option>";
				html +="<option value='4326'>경위도 WGS84 (4326)</option>";
				html +="<option value='4166'>경위도 WGS84 (4166)</option>";
				html +="<option value='5178'>KATEC (5178)</option>";
				html +="<option value='5180'>TM 서부 (5180)</option>";
				html +="<option value='5181'>TM 중부 (5181)</option>";
				html +="<option value='5183'>TM 동부 (5183)</option>";
				html +="<option value='5184'>TM 동해 (5184)'</option>";
				html +="<option value='5185'>TM 서부 (5185)</option>";
				html +="<option value='5186'>TM 중부 (5186)</option>";
				html +="<option value='5187'>TM 동부 (5187)</option>";
				html +="<option value='5188'>TM 동해 (5188)</option>";
			...
			geoCodingStart : function(){
				...
				$dataCreateMain.request.geoCodingStart(geoCodingType,selectColumns,$dataCreateMain.table_id,$dataCreateMain.relation_id,option);
			},
	...
	$dataCreateMain.request = {
			 ...
			 * @history 	 :
			 */
			geoCodingStart : function(geoCodingType,selectColumns,tableId,tableName,option){
