...
(function(W,D){
	...
	$homeMain.request = {
			...
			doReqSaveDashBoardOptList : function() {
				...
				//남은공간/저장공간
				if ($("#checkbox04").is(":checked") == true){
					var save_spacial_yn = "Y"
					dashCnt = dashCnt + 1;
				}else{
					var save_spacial_yn = "N"
				}
				
				//분석데이터
				if ($("#checkbox05").is(":checked") == true){
					var analysis_data_yn = "Y"
					dashCnt = dashCnt + 1;
				}else{
					var analysis_data_yn = "N"
				}
				
				//공유 데이터
				if ($("#checkbox06").is(":checked") == true){
					var share_data_yn = "Y"
					dashCnt = dashCnt + 1;
				}else{
					var share_data_yn = "N"
				...
				console.log(dashCnt);
				if(dashCnt > 5){ // 대시보드 표출 최대 5 항목 제한
					alert('대시보드는 최대 5개 항목까지 표출 가능합니다.');
					return;
				}
