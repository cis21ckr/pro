(function(W,D){
	...
	$(document).ready(function(){
		...
		$dataForm.ui.getusecopyForm();
		$dataForm.event.setUIEvent();	
		$log.srvLogWrite("Z1", "03", "06", "05", "", "");
	});
	
	$dataForm.ui = {
		post_no : null,
		getusecopyForm : function(){
			var obj = {
					params : {
						post_no : $dataForm.ui.post_no
						}
			};
			$dataForm.request.getusecopyForm(obj);
		},
		cancel : function(){
			if(confirm("정말 게시물 작성을 취소하시겠습니까?") == true){
				window.location.href = contextPath + '/view/sysmgt/usecopyLst';
			}
		}
	};
	$dataForm.request = {
		getusecopyForm : function(data){
					...
					$("#boardForm").ajaxForm({
						...
						success: function(res) {
							switch(parseInt(res.errCd)) {
								case 0:
									//log generate by cis	
									$log.srvLogWrite("Z1", "03", "07", "04", "", "");
									
									//목록으로 이동
									window.location.href = "usecopyLst";
									break;
								default:
