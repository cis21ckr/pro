(function(W,D){
	...
	$(document).ready(function(){
		...
		$dataForm.ui.getuseinfoForm();
		$dataForm.event.setUIEvent();	
		$log.srvLogWrite("Z1", "03", "04", "05", "", "");
	});
	
	$dataForm.ui = {
		post_no : null,
		getuseinfoForm : function(){
			var obj = {
				params : {
					post_no : $dataForm.ui.post_no
				}
			};
			$dataForm.request.getuseinfoForm(obj);
		},
		cancel : function(){
			if(confirm("정말 게시물 작성을 취소하시겠습니까?") == true){
				window.location.href = contextPath + '/view/sysmgt/useinfoLst';
			}
		}
	};
	$dataForm.request = {
		getuseinfoForm : function(data){
