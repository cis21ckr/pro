...
(function ($) {
    ...
    var EasyTree = function (jQueryContext, options) {
        ...
        this.activateNode = function (id) {
            ...
            $('#kosis_tree_ul').scrollTop(_settings.position);
        };
        ...
        function iconClick(event) {
        	var id = getElementId(this);
        	var nodes = event.data;
        	var nodeId = id.split("btn_")[1];
            var node = getNode(nodes, nodeId);
            if (!node) { return; }
        	if (_settings.iconSelected) {
            	_settings.iconSelected(event, id, node);
        	}
        }
        ...
        function getNode(nodes, id, text) {
            var i = 0;
        ...
        function buildTree(nodes) {
            ...
            _settings.position = $("#kosis_tree_ul").scrollTop();
            ...
            $($this.selector + " .easytree-info").on("click", nodes, iconClick);
            $($this.selector + " .easytree-req").on("click", nodes, iconClick);
        ...
        function getNodesAsHtml(nodes, level, display) {
            ...
            html += '<ul id="kosis_tree_ul" tabindex="0" class="' + ulCss + '" ' + ulStyle + '">';
            ...
            for (i = 0; i < nodes.length; i++) {
                ...
                html += '<li id=li_' + n.id + '>';
                //2017-09-13 [개발팀] 체크박스 추가
                if (n.checkIcon != undefined && n.checkIcon) {
                	html += '<label id="btn_' + n.id + '" for=id="btn_' + n.id + '" class="easytree-check"></label>'
                }
                ...
                if (n.infoIcon != undefined && n.infoIcon) {
                	html += '<input id="btn_' + n.id + '" type="button" class="easytree-info">';
                }
                
                if (n.reqIcon != undefined && n.reqIcon) {
                	html += '<input id="btn_' + n.id + '" type="button" class="easytree-req">';
                }
