...
(function(W, D) {
	...
	sLegendInfo = {
		    ...
		    geojson.eachLayer(function(layer) {
		    	...
		    	if (layer.feature.info.length > 0) {
		    		if (layer.feature.isKosis) {
				    	...
				    	var kosisData = "";
				    	if (unit != undefined) {
				    		kosisData = searchYear + $commonFunc.appendCommaToNumber(data)+" ("+unit+")";
				    	}else {
				    		kosisData = searchYear + $commonFunc.appendCommaToNumber(data);
				    	}
				    	...
				    	var toolTip  = "<div style='margin:10px;'>";
			    			toolTip += 		"<div style='font-size:14px;font-weight:bold;color:#3792de;'>"+ adm_nm +"</div>";
			    			toolTip +=		"<div style='height:5px;'></div>";
			    			toolTip += 		"<div style='font-size:12px;padding-left:5px;'>"+title+" : "+$commonFunc.appendCommaToNumber(data)+" ("+unit+")</div>";
			    			toolTip += "</div>";
		    		}
		    		else {
		    			info = layer.feature.info[0];		    			
				    	...
				    	var toolTip  = "<div style='margin:10px;'>";
			    			...
			    			if (info.api_id == "API_MYDATA") {
			    				...
			    				if(info.isHouseMap){
			    					...
			    					toolTip += 	"<div style='font-size:12px;padding-left:5px;'>"+searchYear+" "+dataNameText+" : "+$commonFunc.appendCommaToNumber(data)+" ("+unit+")</div>";
			    				}else{
			    					toolTip += 	"<div style='font-size:12px;padding-left:5px;'>"+searchYear+" "+that.getDataName(info.showData)+" : "+$commonFunc.appendCommaToNumber(data)+" ("+unit+")</div>";
			    				}
			    			}
		    	...
		    	if (layer.feature.info.length > 0) {
		    		if (layer.feature.isKosis) {
				    	...
				    	var toolTip  = "<div style='margin:10px;'>";
			    			toolTip += 		"<div style='font-size:14px;font-weight:bold;color:#3792de;'>"+ adm_nm +"</div>";
			    			toolTip +=		"<div style='height:5px;'></div>";
			    			toolTip += 		"<div style='font-size:12px;padding-left:5px;'>"+$commonFunc.appendCommaToNumber(data)+" ("+unit+")</div>";
			    			toolTip += "</div>";
		    		}else {
		    			info = layer.feature.info[0];
			    		data = info[info.showData];
				    	...
				    	var toolTip  = "<div style='margin:10px;'>";
			    			toolTip += 		"<div style='font-size:14px;font-weight:bold;color:#3792de;'>"+ adm_nm +"</div>";
			    			toolTip +=		"<div style='height:5px;'></div>";
			    			toolTip += 		"<div style='font-size:12px;padding-left:5px;'>"+dataNameText+" : "+$commonFunc.appendCommaToNumber(data)+" ("+unit+")</div>";
			    			toolTip += "</div>";
		    		}
	    ...
	    this.removeDataOverlay = function() {
	    	...
	    	if (this.map.pieChartMarkers == null) {
				this.map.pieChartMarkers = [];
			}else {
				for (var i=0; i<this.map.pieChartMarkers.length; i++) {
					this.map.pieChartMarkers[i].remove();
				}
			}
...
//					var colorList2 = getCalculColor("#a501b7", "#7c02cd", that.lv);
		...
		this.setUserLegend = function(type, lv) {
			...
			if (that.valPerSlice == null) {
				tmpValPerSlice = $commonFunc.deepCopy(that.legendValue.user);
			}else {
				if (type == "equal") {
					tmpValPerSlice = $commonFunc.deepCopy(that.legendValue.equal);
				}else if (type == "auto") {
					tmpValPerSlice = $commonFunc.deepCopy(that.legendValue.auto);
				}else {
					tmpValPerSlice = $commonFunc.deepCopy(that.legendValue.user);
				}
			}
		                	   ...
		                	   if (type == "user") {
			                       ...
			                       for (var i=0; i<gChart.series[0].points.length; i++) {
			                           ...
			                           var image = gChart.renderer
			                                   .image(contextPath+'/img/map/grip3.png', parseInt(point.clientX+4)-11 + marginLeft, 10, 15, 110)
			                                   .attr({
			                                       id : "image_"+ i,
			                                       zIndex : "10",
			                                       class : "arrow_grip"
		...
		this.setGoganText = function(valPerSlice){
			...
			if (this.guganTextShow) {
				...
				if (tmpData.length <= 1) {
					for(var i = 0;i <that.lv; i++){
						var idx = (that.lv-1) - i;
						value = $commonFunc.appendCommaToNumber(valPerSlice[i]);
					...
					for(var i = 0;i <that.lv; i++){
						var idx = (that.lv-1) - i;
						if (i == 0) {
							value = $commonFunc.appendCommaToNumber(valPerSlice[i+1]) + " 이하"; //2016.03.29 수정, 이하 삭제
						}else if (i == that.lv-1) {
							value = $commonFunc.appendCommaToNumber(valPerSlice[i]) + " 초과";
						}else {
							value = $commonFunc.appendCommaToNumber(valPerSlice[i]) + " ~ " + $commonFunc.appendCommaToNumber(valPerSlice[i+1]) + " 이하"; //2016.03.29 수정, 이하
						}
						$("#colorStatus_"+that.id+">li:eq("+idx+")>span").text(value);
				...
				open: function( event, ui ) {
					...
					setTimeout(function() {
						$(".ui-tooltip .subj").text(target.attr("data-subj"));
						 ...
						 if(target.attr("data-subj") == "집계구 위치 찾기")
							 ui.tooltip.css("top", event.clientY - 10);
		...
		this.changeUserLegend = function (lv, valPerSlice, userTableWidthList) {
			...
			that.tmpValPerSlice = $commonFunc.deepCopy(valPerSlice[0]);
		...
		this.legendCopy = function(targetId) {
			...
			if (this.legendList == null) {
				...
				for (var i=0; i<this.delegate.mapList.length; i++) {
					...
					this.legendList[i] = $commonFunc.deepCopy(legendInfo);
				}
			}else {
				for (var i=0; i<this.delegate.mapList.length; i++) {
					var map = this.delegate.mapList[i];
		...
		this.createLegend = function() {
			...
			infoLegend.update = function(props) {
				...
				this._div.innerHTML = '<div data-html2canvas-ignore=true class="legendRing legendBox max" id="legendBox_'+that.id+'" data-ing="max">'
						...
						+colorList
						+'</ul>'
						+'<div class="heatMin" id="heatMinLegend_'+that.id +'" style="display:none;"><img src="'+contextPath+'/img/map/wheel.500.png" style="width:80px;height:80px;margin:14px 10px 10px 10px;"/></div>'
						...
						+'<img src="'+contextPath+'/img/map/ico_dbPlay.png" style="float:left;margin:4px 10px 0 10px;width:10px;height:10px;"/><div style="height:20px;font-size:11px;">반지름 조절 (Radius)</div>'
						...
						+'</ul>'
						+'</div>'
						+'<img src="'+contextPath+'/img/map/ico_dbPlay.png" width=10 height=10 style="float:left;margin:4px 10px 0 10px;width:10px;height:10px;"/><div style="height:20px;font-size:11px;">흐림도 조절 (Blur)</div>'
						+'<div style="height:50px;">'
	...
	for ( var i = 0; i < arrColor.length; i++) {
		...
		if (obj.valPerSlice != null) {
			if ($(cls).parent().attr("class") == "ring") {
				...
				$("#bubbleLegendLine_"+id).find("li").eq(9-i).find("span").html($commonFunc.appendCommaToNumber(obj.valPerSlice[0][i]));
			}else {
