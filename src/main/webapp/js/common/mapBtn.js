	...
	$mapBtn = {
		btnInfo	: function(map, delegate) {
			...
			this.createUI = function(options) {
				...
				this.id = $commonFunc.makeStamp(currentdate);
				...
				if (options != null) {
					...
					if (options.searchControl != undefined && options.searchControl) {
						this.createSearchControl("polygon");
						this.createSettingBtn("polygon");
					}
					
					if (options.searchPoiControl != undefined && options.searchPoiControl) {
						this.createSearchControl("poi");
						this.createSettingBtn("poi");
					}
					
					if (options.searchAnalysisControl != undefined && options.searchAnalysisControl) {
						this.createSearchControl("analysis");
						this.createSettingBtn("analysis");
					}
					
					if (options.searchAnalysisPoiControl != undefined && options.searchAnalysisPoiControl) {
						this.createSearchControl("analysisPoi");
						this.createSettingBtn("analysis");
					}
			...
			this.createMapTypeBtn = function() {
				...
				mapUI.update = function (props) {
					...
					//html += 	"<li><a name='settlite' class='ico_side_gps01'><span>위성</span></a></li>";  // 190225 위성 제거
			...
			this.createSearchControl = function(type) {
				var searchUI = sop.control({position: 'topleft'});
				searchUI.onAdd = function (map) {
				    this._div = sop.DomUtil.create('div', 'info');
				    sop.DomEvent.disableClickPropagation(this._div);
				    this.update();
				    $(this._div).attr("id", 'search_' +that.id);
				    that.searchUI = this._div;
				    return this._div;
				};
				searchUI.update = function (props) {
					var width = $(".sop-container").width();
					var html = "";
					html += "<div data-html2canvas-ignore=true class='mapSearchWrapper' style='width:"+width+"px;'>";
					html +=	"<div style='margin:0 auto;'>";
					
					if (type != "analysis" && type != "analysisPoi") {
						html += "<input type='text' placeholder='주소를 입력하세요.' id='mapSearch_"+that.id+"' class='mapSearch' />";
						html += "<button class='btnSearch'  id='btnSearch"+that.id+"'></button>";
					}
					
					switch(type) {
						case "polygon" :
							html += "<button id='drawPolygon_"+that.id+"' class='drawBtn'>영역그리기</button>";
							break;
						case "poi" :
							html += "<button id='drawPoi_"+that.id+"' class='drawBtn'>POI 생성하기</button>";
							break;
						case "analysis" :
							html += "<button id='drawEach_"+that.id+"' class='drawBtn'>개별선택</button>";
							html += "<button id='drawRectangle_"+that.id+"' class='drawBtn'>영역선택</button>";
							break;
						case "analysisPoi" :
							html += "<button id='drawEach_"+that.id+"' class='drawBtn'>개별선택</button>";
							break;
					...
					html += "<button id='initPolygon_"+that.id+"' class='drawBtn'>초기화</button>";
					html +=	"</div>";
					html += "</div>";
					this._div.innerHTML = html;
					 
				};
				searchUI.addTo(this.map.gMap);
				
				//검색버튼 선택
				$("#searchBtn"+that.id).on("click", function() {
					var searchNm = $("#mapSearch_"+that.id).val();
					if (searchNm.length == 0) {
						$message.open("알림", "검색어를 입력해주세요.");
						return;
					}
					that.reqSearchAddres(searchNm, function(result, map) {
						if ($.isFunction(delegate.ui.drawMarker)) {
							delegate.ui.drawMarker(result.x, result.y, map);
						}
					});
				});
				
				//검색버튼 선택(키보드)
				$("#mapSearch_"+that.id).keydown(function(e){
					if (e.keyCode == 13) {
						var searchNm = $("#mapSearch_"+that.id).val();
						if (searchNm.length == 0) {
							$message.open("알림", "검색어를 입력해주세요.");
				...
				//영역그리기
				$("#drawPolygon_"+that.id).on("click", function(e){
					$message.open("알림", "지도상에 클릭 또는 드래그로 영역을 그리세요.", function() {
						if (that.delegate && 
								that.delegate.callbackFunc && 
								that.delegate.callbackFunc.didDrawPolygonChangeMode instanceof Function) {
								that.delegate.callbackFunc.didDrawPolygonChangeMode(that.map);
							}
					});
				});
				
				//POI 생성하기
				$("#drawPoi_"+that.id).on("click", function(e){
					if (that.delegate && 
						that.delegate.callbackFunc && 
						that.delegate.callbackFunc.didDrawPoi instanceof Function) {
						that.delegate.callbackFunc.didDrawPoi(that.map);
					}
				});
				
				//개별선택
				$("#drawEach_"+that.id).on("click", function(e){
					$message.open("알림", "개별로 폴리곤 또는 POI를 선택합니다.", function() {
						if (that.delegate && 
							that.delegate.callbackFunc && 
							that.delegate.callbackFunc.didDrawEachMode instanceof Function) {
							that.delegate.callbackFunc.didDrawEachMode(that.map);
						}
					});
				});
				
				//영역(사각형)선택
				$("#drawRectangle_"+that.id).on("click", function(e){
					$message.open("알림", "영역을 그려 영역안의 폴리곤 또는 POI를 선택합니다.", function() {
						if (that.delegate && 
								that.delegate.callbackFunc && 
								that.delegate.callbackFunc.didDrawPolygonChangeMode instanceof Function) {
								that.delegate.callbackFunc.didDrawPolygonChangeMode(that.map, "rectangle");
							}
					});
				});
				...
				//초기화
				$("#initPolygon_"+that.id).on("click", function(e){
					if (that.delegate && 
						that.delegate.callbackFunc && 
						that.delegate.callbackFunc.didInitPolygonData instanceof Function) {
						that.delegate.callbackFunc.didInitPolygonData(that.map);
					}
				});	
			},
			...
			this.addLifePoiList = function() {
				var html = "";
				html += "<li><a name='1001'><img src='/img/map/ico_1001.png' alt='인테리어'/><span style='cursor:pointer;'>인테리어</span></a></li>";	
				html += "<li><a name='1002'><img src='/img/map/ico_1002.png' alt='목욕탕'/><span style='cursor:pointer;'>목욕탕</span></a></li>";
				html += "<li><a name='1003'><img src='/img/map/ico_1003.png' alt='교습학원'/><span style='cursor:pointer;'>교습학원</span></a></li>";
				html += "<li><a name='1004'><img src='/img/map/ico_1004.png' alt='어학원'/><span style='cursor:pointer;'>어학원</span></a></li>";
				html += "<li><a name='1005'><img src='/img/map/ico_1005.png' alt='예체능학원'/><span style='cursor:pointer;'>예체능학원</span></a></li>";
				html += "<li><a name='1006'><img src='/img/map/ico_1006.png' alt='부동산중개업'/><span style='cursor:pointer;'>부동산중개업</span></a></li>";
				html += "<li><a name='1007'><img src='/img/map/ico_1007.png' alt='이발소'/><span style='cursor:pointer;'>이발소</span></a></li>";    		
				html += "<li><a name='1008'><img src='/img/map/ico_1008.png' alt='미용실'/><span style='cursor:pointer;'>미용실</span></a></li>";     			
				html += "<li><a name='1009'><img src='/img/map/ico_1009.png' alt='세탁소'/><span style='cursor:pointer;'>세탁소</span></a></li>";    				
				html += "<li><a name='1010'><img src='/img/map/ico_1010.png' alt='PC방'/><span style='cursor:pointer;'>PC방</span></a></li>";    				
				html += "<li><a name='1011'><img src='/img/map/ico_1011.png' alt='노래방'/><span style='cursor:pointer;'>노래방</span></a></li>";     								
			...
			this.addShopPoiList = function() {
				var html = "";
				html += "<li><a name='2001'><img src='/img/map/ico_2001.png' alt='문구점'/><span style='cursor:pointer;'>문구점</span></a></li>";        				
				html += "<li><a name='2002'><img src='/img/map/ico_2002.png' alt='서점'/><span style='cursor:pointer;'>서점</span></a></li>";    				
				html += "<li><a name='2003'><img src='/img/map/ico_2003.png' alt='편의점'/><span style='cursor:pointer;'>편의점</span></a></li>";    				
				html += "<li><a name='2004'><img src='/img/map/ico_2004.png' alt='식료품점'/><span style='cursor:pointer;'>식료품점</span></a></li>";    				
				html += "<li><a name='2005'><img src='/img/map/ico_2005.png' alt='휴대폰점'/><span style='cursor:pointer;'>휴대폰점</span></a></li>";  
				html += "<li><a name='2006'><img src='/img/map/ico_2006.png' alt='의류'/><span style='cursor:pointer;'>의류</span></a></li>";
				html += "<li><a name='2007'><img src='/img/map/ico_2007.png' alt='화장품/방향제'/><span style='cursor:pointer;'>화장품/방향제</span></a></li>";
				html += "<li><a name='2008'><img src='/img/map/ico_2008.png' alt='철물점'/><span style='cursor:pointer;'>철물점</span></a></li>";
				html += "<li><a name='2009'><img src='/img/map/ico_2009.png' alt='주유소'/><span style='cursor:pointer;'>주유소</span></a></li>";
				html += "<li><a name='2010'><img src='/img/map/ico_2010.png' alt='꽃집'/><span style='cursor:pointer;'>꽃집</span></a></li>";
				html += "<li><a name='2011'><img src='/img/map/ico_2011.png' alt='슈퍼마켓'/><span style='cursor:pointer;'>슈퍼마켓</span></a></li>";
			...
			this.addTrafficPoiList = function() {
				var html = "";
				html += "<li><a name='3001'><img src='/img/map/ico_3001.png' alt='지하철역'/><span style='cursor:pointer;'>지하철역</span></a></li>";    				
				html += "<li><a name='3002'><img src='/img/map/ico_3002.png' alt='터미널'/><span style='cursor:pointer;'>터미널</span></a></li>";    						
			...
			this.addLodgePoiList = function() {
				var html = "";
				html += "<li><a name='4001'><img src='/img/map/ico_4001.png' alt='호텔'/><span style='cursor:pointer;'>호텔</span></a></li>";
				html += "<li><a name='4002'><img src='/img/map/ico_4002.png' alt='여관'/><span style='cursor:pointer;'>여관</span></a></li>"; 	
				html += "<li><a name='4003'><img src='/img/map/ico_4003.png' alt='펜션'/><span style='cursor:pointer;'>펜션</span></a></li>";
			...
			this.addRestaurantPoiList = function() {
				var html = "";
				html += "<li><a name='5001'><img src='/img/map/ico_5001.png' alt='한식'/><span style='cursor:pointer;'>한식</span></a></li>";
				html += "<li><a name='5002'><img src='/img/map/ico_5002.png' alt='중식'/><span style='cursor:pointer;'>중식</span></a></li>";
				html += "<li><a name='5003'><img src='/img/map/ico_5003.png' alt='일식'/><span style='cursor:pointer;'>일식</span></a></li>";
				html += "<li><a name='5004'><img src='/img/map/ico_5004.png' alt='분식'/><span style='cursor:pointer;'>분식</span></a></li>";
				html += "<li><a name='5005'><img src='/img/map/ico_5005.png' alt='서양식'/><span style='cursor:pointer;'>서양식</span></a></li>";
				html += "<li><a name='5006'><img src='/img/map/ico_5006.png' alt='제과점'/><span style='cursor:pointer;'>제과점</span></a></li>";
				html += "<li><a name='5007'><img src='/img/map/ico_5007.png' alt='패스트푸드'/><span style='cursor:pointer;'>패스트푸드</span></a></li>";
				html += "<li><a name='5008'><img src='/img/map/ico_5008.png' alt='치킨'/><span style='cursor:pointer;'>치킨</span></a></li>";
				html += "<li><a name='5009'><img src='/img/map/ico_5009.png' alt='호프/간이주점'/><span style='cursor:pointer;'>호프/간이주점</span></a></li>";
				html += "<li><a name='5010'><img src='/img/map/ico_5010.png' alt='카페'/><span style='cursor:pointer;'>카페</span></a></li>";
				html += "<li><a name='5011'><img src='/img/map/ico_5011.png' alt='기타 외국식'/><span style='cursor:pointer;'>기타 외국식</span></a></li>"; 
			...
			this.addPublicPoiList = function() {
				var html = "";
				html += "<li><a name='6001'><img src='/img/map/ico_6001.png' alt='우체국'/><span style='cursor:pointer;'>우체국</span></a></li>";
				html += "<li><a name='6002'><img src='/img/map/ico_6002.png' alt='행정기관'/><span style='cursor:pointer;'>행정기관</span></a></li>";
				html += "<li><a name='6003'><img src='/img/map/ico_6003.png' alt='경찰/지구대'/><span style='cursor:pointer;'>경찰/지구대</span></a></li>";
				html += "<li><a name='6004'><img src='/img/map/ico_6004.png' alt='소방서'/><span style='cursor:pointer;'>소방서</span></a></li>"; 
			...
			this.addEduPoiList = function() {
				var html = "";
				html += "<li><a name='7001'><img src='/img/map/ico_7001.png' alt='초등학교'/><span style='cursor:pointer;'>초등학교</span></a></li>";
				html += "<li><a name='7002'><img src='/img/map/ico_7001.png' alt='중학교'/><span style='cursor:pointer;'>중학교</span></a></li>";
				html += "<li><a name='7003'><img src='/img/map/ico_7001.png' alt='고등학교'/><span style='cursor:pointer;'>고등학교</span></a></li>";
				html += "<li><a name='7004'><img src='/img/map/ico_7001.png' alt='전문대학'/><span style='cursor:pointer;'>전문대학</span></a></li>";
				html += "<li><a name='7005'><img src='/img/map/ico_7001.png' alt='대학교'/><span style='cursor:pointer;'>대학교</span></a></li>";
				html += "<li><a name='7006'><img src='/img/map/ico_7001.png' alt='대학원'/><span style='cursor:pointer;'>대학원</span></a></li>";
				html += "<li><a name='7007'><img src='/img/map/ico_7001.png' alt='어린이보육업'/><span style='cursor:pointer;'>어린이보육업</span></a></li>";
			...
			this.addCompanyPoiList = function() {
				var html = "";
				html += "<li><a name='8001'><img src='/img/map/ico_8001.png' alt='제조/화학'/><span style='cursor:pointer;'>제조/화학</span></a></li>";
				html += "<li><a name='8002'><img src='/img/map/ico_8002.png' alt='서비스'/><span style='cursor:pointer;'>서비스</span></a></li>";
				html += "<li><a name='8003'><img src='/img/map/ico_8003.png' alt='통신/IT'/><span style='cursor:pointer;'>통신/IT</span></a></li>";
				html += "<li><a name='8004'><img src='/img/map/ico_8004.png' alt='건설'/><span style='cursor:pointer;'>건설</span></a></li>";
				html += "<li><a name='8005'><img src='/img/map/ico_8005.png' alt='판매/유통'/><span style='cursor:pointer;'>판매/유통</span></a></li>";
				html += "<li><a name='8006'><img src='/img/map/ico_8006.png' alt='기타금융업'/><span style='cursor:pointer;'>기타금융업</span></a></li>";
				html += "<li><a name='8007'><img src='/img/map/ico_8007.png' alt='기타의료업'/><span style='cursor:pointer;'>기타의료업</span></a></li>";
				html += "<li><a name='8008'><img src='/img/map/ico_8008.png' alt='문화/체육'/><span style='cursor:pointer;'>문화/체육</span></a></li>";
			...
			this.addCulturePoiList = function() {
				var html = "";
				html += "<li><a name='9001'><img src='/img/map/ico_9001.png' alt='백화점/중대형마트'/><span style='cursor:pointer;'>백화점/중대형마트</span></a></li>";
				html += "<li><a name='9002'><img src='/img/map/ico_9002.png' alt='은행'/><span>은행</span style='cursor:pointer;'></a></li>";
				html += "<li><a name='9003'><img src='/img/map/ico_9003.png' alt='병원'/><span>병원</span style='cursor:pointer;'></a></li>";
				html += "<li><a name='9004'><img src='/img/map/ico_9004.png' alt='극장/영화관'/><span style='cursor:pointer;'>극장/영화관</span></a></li>";
				html += "<li><a name='9005'><img src='/img/map/ico_9005.png' alt='도서관/박물관'/><span style='cursor:pointer;'>도서관/박물관</span></a></li>";
			...
			this.eventHandler = function() {
				...
				$("#zoom_" + that.id).find(".rightQuick.rq07").click(function(){
					...
					if(!on){
						$(this).addClass("on");
						$(this).css("background-image","url("+contextPath+"/img/map/ico_moveType02.png)");
						...
						$(this).removeClass("on");
						$(this).css("background-image","url("+contextPath+"/img/map/ico_moveType01.png)");
			...
			this.reqThemePoiInfo = function(themeCd, pageNum) {
				...
				$ajax.requestApi(openApiPath + "/OpenAPI3/stats/companysearch.json", options,  function(res) {
					switch(parseInt(res.errCd)) {
						case 0:
							...
							if (res.errCd == "0") {
								...
								var apicallCount = parseInt(totalCount / (parseInt(options.params.resultcount) * (pageNum + 1)));

								if (returnCount !== totalCount && apicallCount > 0) {
										that.reqThemePoiInfo(options.params.theme_cd, pageNum + 1);
								}
								
								var theme_cd = "";
								if (options.params.theme_cd.length == 2) {
									theme_cd = options.params.theme_cd;
								}else {
									theme_cd = options.params.theme_cd.substring(0, 2) + '_' + options.params.theme_cd.substring(2, 4);
								}
				...
				$ajax.requestApi(openApiPath + "/OpenAPI3/stats/companysearch.json", options,  function(res) {
					switch(parseInt(res.errCd)) {
						case 0:
							...
							if (returnCount !== totalCount && apicallCount > 0) {
									that.reqCompanyPoiInfo(options.params.class_code, options.params.class_deg, pageNum + 1);
							}
					...
					switch(parseInt(res.errCd)) {
						case 0:
							...
							for(var i=0; i < result.length; i++) {
								var tmpObj = {};
								tmpObj.id = that.id + "_" + result[i].class_code + "_" + depth;
							...
							if (that.companyTree == null) {
											...
											if (node.childCount == null) {
												if (node.children.length > 0 ) {
													if(node.children[0].id == node.id + "_progress") {
														if (node.isExpanded) {
															that.reqInterstryList(node.depth, class_deg, node.cd);
														}
													}
												}
											}
								...
								for (var i=0; i<tmpData.length; i++) {
									that.companyTree.addNode(tmpData[i], that.id + "_" + class_cd + "_" +(depth-1));
								}
								that.companyTree.removeNode( that.id + "_" +class_cd + "_" + (depth-1) + "_progress");
								that.companyTree.rebuildTree();
							}
							break;
						case -401:
							that.reqInterstryList(depth, class_deg, class_cd);
							break;
						default:
							break;
					}
			...
			this.reqSearchAddres = function(searchNm, callback) {
				var options = {
						isBeforSend : false,
						method : "GET",
						params : {
				...
				$ajax.requestApi(openApiPath + "/OpenAPI3/addr/geocode.json", options,  function(res) {
					switch(parseInt(res.errCd)) {
						case 0:
							if (res.result.resultdata.length > 0) {
								var result = res.result.resultdata[0];
								that.map.mapMove([result.x, result.y], 11);
								
								if (callback != undefined && callback != null && typeof callback === "function") {
									callback.call(undefined, result, that.map);
								}
							}
							break;
						case -401:
							that.reqSearchAddres(searcgNm, callback)
							break;
						default:
							break;
					}
