...
Draw.DistanceMeasure = Draw.Feature.extend({
	//polyline Option
	shapeOptions: {
		stroke: true,
		color: '#6799FF',
	...
	_createSubUI: function (element) {
		this.ui = {};
		this.ui = sop.DomUtil.create('li', 'sop-distance-out', element);
	...
	_switchSubOnButton: function () {
		...
		sop.DomUtil.removeClass(this.ui, 'draw-distance-out');
		sop.DomUtil.addClass(this.ui, 'draw-distance-selected');
	},

	//button 클릭시 이미지 변경 Off
	_switchSubOffButton: function () {
		sop.DomUtil.removeClass(this.ui, 'draw-distance-selected');
		sop.DomUtil.addClass(this.ui, 'draw-distance-out');
	},
		...
		_closeMarker.on('click', function () {
			...
			for (var i=0; i<markersGroupLayers.length;i++) {
				if (markersGroupLayers[i].hasLayer(_closeMarker)) {
					...
					this._polylineGroup.removeLayer(this._polylineGroup.getLayers()[i]);
