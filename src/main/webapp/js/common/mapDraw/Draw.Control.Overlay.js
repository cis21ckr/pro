 ...
 */
Draw.Control.Overlay = Draw.Feature.extend({
	//Control 이름
	...
	initMeasureSubControl: function (_map) {
		this.measure={};
		this.measure.circle = new Draw.Circle(_map);
		this.measure.rectangle = new Draw.Rectangle(_map);
		this.measure.polygon = new Draw.Polygon(_map);
	},
	...
	_createUI: function (element) {
		...
		this.ui = {};
		this.ui = sop.DomUtil.create('div', 'draw-overlay-out', element);
		this.ui.title='사용자정의 통계조회';
	...
	_switchOnButton: function () {
		sop.DomUtil.removeClass(this.ui, 'draw-overlay-out');
		//sop.DomUtil.addClass(this.ui, 'draw-overlay-out');
		 sop.DomUtil.addClass(this.ui, 'draw-overlay-selected');
	},

	//button 클릭시 이미지 변경 Off
	_switchOffButton: function () {
		sop.DomUtil.addClass(this.ui, 'draw-overlay-out');
		sop.DomUtil.removeClass(this.ui, 'draw-overlay-selected');
	//	sop.DomUtil.addClass(this.ui, 'draw-overlay-out');
	},
