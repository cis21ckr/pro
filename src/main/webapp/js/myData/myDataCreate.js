	...
	$myDataCreate.ui = {
		...
		processStep : function(step) {
			...
			case "1":
				...
				var description = $("textarea[name='show_description']").val();
				if ($.trim(tableName) == "" || $.trim(description) == "") {
					$message.open('알림', "테이블 제목과 설명을 입력 하세요.");
					$myDataCreate.event.prevStep("2");
				} else {
					$("input:text[name='output_table_name']").val(tableName);
					$("textarea[name='description']").val(description);

					// file검사 추가 해야 함
					var type = $('input:radio[name="data_type"]:checked').val();
					...
					//table이름 중복 검사
					 ...
					 $.ajax({
							...
							success : function(res){
								...
								if(res.count > 0){
									$message.open("알림","중복된 이름의 테이블이 존재 합니다.");
									$myDataCreate.event.prevStep("2");
								}else{
									if(type == "SHP"){
										//SHP 일때는 바로 테이블 생성
										
										if($('input:file[name="multiFile"]').eq(0).val() ==undefined || $('input:file[name="multiFile"]').eq(1).val() ==undefined || $('input:file[name="multiFile"]').eq(2).val() == undefined){
											$message.open('알림', "파일을 선택 해주세요.");
											$myDataCreate.event.prevStep("2");
										}else{
											$myDataCreate.request.createTable();
										}
			...
			case "3":
				...
				$myDataCreate.request.geoCoding(type , "");
		...
		gridPreviewData : function(type, data, header, headerList) {
			...
			// [{name : name , type : type}]
			if (header == true) {
				$myDataCreate.ui.columnGridData(headerList);
			} else {
				var columnHeader = data[0];
				headerList = new Array();
				for (var i = 0; i < columnHeader.length; i++) {
					var key = Number(i) + 1;
					headerList.push("item" + key);
				}
	...
	$myDataCreate.event = {
		setUIEvent : function() {
			...
			$(".prevButton").off().on("click", function() {
				var id = $(this).attr("id");
				var step = id.split("_")[1];
				$myDataCreate.event.prevStep(step);
			...
			$('input:radio[name="show_charsets"]').off().on("click",
					function() {
						var val = $(this).val();
						$myDataCreate.event.changeEncodingType(val);
					});

			$('input:checkbox[name="show_headerCheck"]').off().on("click",
					function() {
						$myDataCreate.event.chageHeaderCheck();
					});
			
			$('select[name="show_delimiter"]').off().on("change",function(){
				var selectVal = $('select[name="show_delimiter"]').val();
				console.log(selectVal);
				$("input[name='delimiter']").val(selectVal);
				console.log($("input[name='delimiter']").val());
				$myDataCreate.request.previewData($myDataCreate.ui.gridPreviewData,0, 5);
			});
		...
		nextStep : function(step) {
			...
			if(step == "3"){
				...
				if(type == "geom"){
					var columnList = JSON.parse($("input[name='columnInfos']").val());
					var geomValidate = true;
					for(var i = 0 ; i < columnList.length; i++){
						var column = columnList[i];
						if(column.column_name == $("select[name='selectColumn']").eq(0).val().toLowerCase()){
							if(column.data_type != "USER-DEFINED"){
								$message.open("알림","geometry로 변환 할수 없는 컬럼 입니다.");
								geomValidate = false;
								return;
							}
						}
			...
			if(step == "4"){
				location.href = contextPath+"/view/analysis/analysisMain";
			}
			$("#stepForm_" + step).hide();
			var nextStep = Number(step) + 1;
			$("#stepForm_" + nextStep).show();
		...
		prevStep : function(step) {
			...
			if (step != 1) {
				if(step == 4){
					...
					//삭제
					var dataId = $("#gridGeoCodingTable").data("resource_id");
					if(dataId == undefined || dataId == null){
						var dataNm = $("input[name='output_table_name']").val();
						$myDataCreate.request.deleteErrorMyData(dataNm);
					}else{
						var deleteDatas = new Array();
						deleteDatas.push(dataId);
						$myDataCreate.request.deleteDatas(deleteDatas);
					}
				
					var prevStep = Number(step) - 1;
					$("#stepForm_" + prevStep).show();
		...
		/**
		 * 
		 * @name : changeEncodingType
		 * @description : 인코딩 타입 변경
		 * @date : 2018.08.01
		 * @author :
		 * @history :
		 */
		changeEncodingType : function(type) {
			$('input:radio[name="charsets"]').attr('checked', false);
			$('input:radio[name="charsets"]').filter('[value="' + type + '"]')
					.attr('checked', "checked");
			$('input:radio[name="charsets"]').filter('[value="' + type + '"]')
					.prop('checked', "checked");
			$myDataCreate.request.previewData($myDataCreate.ui.gridPreviewData,
					0, 5);
		},

		/**
		 * 
		 * @name : chageHeaderCheck
		 * @description : 첫줄 포함 변경
		 * @date : 2018.08.01
		 * @author :
		 * @history :
		 */
		chageHeaderCheck : function() {
			$('input:checkbox[name="headerCheck"]').trigger("click");
			$myDataCreate.request.previewData($myDataCreate.ui.gridPreviewData,
					0, 5);
		},
		...
		/**
		 * 
		 * @name : geoCodingColumnClick
		 * @description : 지오코딩 타입 선택시 이벤트 선택
		 * @date : 2018.08.01
		 * @author :
		 * @history :
		 */
		geoCodingColumnClick : function(type) {
			$(".geoChkRadio").removeClass("on");
			$("#" + type + "Chk").addClass("on");
			$myDataCreate.ui.geoCodingSettingBarGrid(type);
