	...
	$sgisDataMng.event = {
		setUIEvent : function(){
			...
			$("#schema").on("change", function() {
				$sgisDataMng.request.doReqSgisDataMngInfo($sgisDataMng.ui.db_type);		
			});
					...
					switch(parseInt(res.errCd)) {
						case 0:
							...
							for (var arri=0;arri<data.length;arri++) {
								...
								html += "<tr><td table_nm='"+column_name+"'>";
								html += column_name;
								if (obj.data_type!="") {
			...
			$("#btnSelect10").on("click", function() {
				...
				window.editor.setValue("select * from " + $("#schema").val() + "." + $sgisDataMng.ui.table_nm + " limit 10;");
			});
			...
			$("#btnSelectCount").on("click", function() {
				...
				window.editor.setValue("select count(*) as cnt from " + $("#schema").val() + "." + $sgisDataMng.ui.table_nm + " limit 1;");
			});
			...
			$("#tableDownload").on("click", function() {
				if ($sgisDataMng.ui.table_nm == '') return;
				if ($sgisDataMng.ui.db_type=="hive") {
					location.href = contextPath + "/api/datasvc/downloadHive.do?schemaNm=" + $("#schema").val() + "&tableNm=" + $sgisDataMng.ui.table_nm;
				} else {
					location.href = contextPath + "/api/sysmgt/downloadPG.do?schemaNm=" + $("#schema").val() + "&tableNm=" + $sgisDataMng.ui.table_nm;
				}
			});
			...
			$("#resultDownload").on("click", function() {
				...
				$log.srvLogWrite("Z2", "05", "01", "08", "", "");
				if ($sgisDataMng.ui.db_type=="hive") {
					location.href = contextPath + "/api/datasvc/downloadHive.do?schemaNm=" + $("#schema").val() + "&qryTxt=" + qry_txt + "&tableNm=" + $sgisDataMng.ui.table_nm;
				} else {
					location.href = contextPath + "/api/sysmgt/downloadPG.do?schemaNm=" + $("#schema").val() + "&qryTxt=" + qry_txt;
				}
			});			
			...
			$("#tableTransfer").on("click", function() {
				...
				var url_href = contextPath + "/api/datasvc/" + url + ".do?schemaNm=" + $("#schema").val() + "&tableNm=" + $sgisDataMng.ui.table_nm;
