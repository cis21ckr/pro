	...
	$workSet.ui = {
			...
			setworkSetListTable : function(data, pageSize, curPage) {
				...
				for (var i=0; i<data.length; i++) {
					...
					html +=	"<tr id='sIdx_"+i+"'>";
					html +=		"<td><span class='checkbox solo'><input type='checkbox' name='job_seq' id='c" + i +"' value='"+data[i].job_setup_seq+"'><label for='c" + i +"'>&nbsp;</label></span></td>";
					html +=		"<td>"+(no+1)+"</td>";
					html +=		"<td class='left'><a href='"+ contextPath +"/view/prjMng/workSetDetail?job_setup_seq="+data[i].job_setup_seq+"'>"+data[i].job_nm+"</a></td>";
