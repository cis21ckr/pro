...
(function(W,D){
	...
	$prjExec.ui = {
			...
			setParams : function(startIdx) {
				...
				var clNm = $("#clNm option:selected").val();
				...
				if (clNm.length > 0) {
					options.params["clNm"] = clNm;
				}
				options.params["stateCd"] = $prjExec.ui.stateCd;
				return options;
			},
			...
			prjSetListViewPaging : function(totalCount, totalPage, pageSize, data, pageIndex) {
				...
				$('#prjSetPage').paging({
					...
					onclick : function(e,page){
						$prjExec.ui.srtIdx = (page - 1) * pageSize;
						$prjExec.ui.currentPage = page;
						$prjExec.request.doReqprjSetList($prjExec.ui.srtIdx);
					}
				});
			},
			...
			setprjSetListTable : function(data, pageSize, curPage) {
				...
				for (var i=0; i<data.length; i++) {
					...
					html +=	"<tr id='sIdx_"+i+"'><td class='td-projectnum' style='width:100px;'>1</td><td class='tb-projectlist'><div class='projectlist-tit'><div class='text'>";
