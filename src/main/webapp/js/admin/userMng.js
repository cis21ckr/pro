(function(W,D){
	...
	$userMng.request = {
			deletePost : function(inst_seq){
				$.ajax({
					type : "POST",
					url : contextPath +"/api/sysmgt/deleteUserMng.do",
					dataType : "json",
					sync : true,
						...
						case 0:
							...
							if (result.length > 0) {
								...
								for(var i=0; i < result.length;i++){
									...
									html = html + '<a class="notice-title" href="' + contextPath + '/view/sysmgt/userMngEdit?user_no='+result[i].user_no+'">'+result[i].user_nm+'</a></td>';
