...
public class CommonAPI {
	...
	@Interceptor("PageCallReg")
	@RequestMapping(value="/saveCaptureImage.do")
	public ModelAndView saveCaptureImage(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		try {
			...
			if (data == null) {
				throw new AuthorityException("필수파라미터가 누락되었습니다.");
			}
			...
			if(FileUtils.writeFileForBytes(decode, filePath, fileId, "png")) {
				results.put("attach", fileId);
				results.put("path", "share");
				results.put("extention", "png");
				results.put("file_nm", fileId + ".png");
				results.put("file_size", decode.length);
			}
			
			model.put("id", "G2G50002");
			model.put("errCd", "0");
			model.put("errMsg", "Success");
			model.put("result", results);
