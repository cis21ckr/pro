...
public class DataCreateAPI {
	 ...
	 */
	@RequestMapping(value = "/readShp", method = RequestMethod.POST)
	public ModelAndView readShp(MultipartHttpServletRequest fileRequest, HttpServletRequest request)
			throws MalformedURLException, IOException, FactoryException {
		/*Map map = new HashMap();*/
		Iterator<String> iter = fileRequest.getFileNames();
		ShpFileUtil shpUtil = new ShpFileUtil();
		ModelMap map = new ModelMap();
		String encodingType = request.getParameter("encoding_type");
		if(encodingType.equals("CP949")) {
		    encodingType = "EUC-KR";
		}
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < files.size(); i++) {
				...
				String extType = file.getOriginalFilename().split("\\.")[1];
				if (extType.equalsIgnoreCase("shp")) {
					map.put("shpList", shpUtil.ShpFileRead(file));
				} else if (extType.equalsIgnoreCase("dbf")) {
					map.put("dbfMap", shpUtil.dbfFileRead(file, encodingType));
				} else if (extType.equalsIgnoreCase("shx")) {
					map.put("shapeInfo", shpUtil.shxFileRead(file));
				} else if (extType.equalsIgnoreCase("prj")) {
					map.put("wktName", shpUtil.prjFileRead(file));
				}
			}
		}
		
		System.out.println("shpFile 전송 성공");
		System.out.println(map);
		return new ModelAndView("jsonV",map);
	}
