...
@Service("locationRankAnalysisService")
public class LocationRankAnalysisServiceImpl extends EgovAbstractServiceImpl implements LocationRankAnalysisService {
...
//	private PostgresService postgreService;
	private static final Logger logger = LoggerFactory.getLogger(LocationRankAnalysisService.class);
	...
	public JSONObject rankAnalysis(MyDataAnalysis myDataAnalysis, JSONArray condition_list)
			throws SystemFailException { 
		
		myDataAnalysis = this.setAdmTypeByAreaCode(myDataAnalysis);
		...
		//GEO_DATA_TYPE =======================================
		parameters.put("GEO_DATA_TYPE", "LOCATION_RANK_DATA");
		...
		// POLYGON_TABLE  ========================================
		parameters = this.insertParametersByAdmType(myDataAnalysis, parameters);
	...
	private MyDataAnalysis setAdmTypeByAreaCode(MyDataAnalysis data) throws SystemFailException {
		if (data.getArea_cd() != null) {
			if ("all".equalsIgnoreCase(data.getArea_cd())) {
				this.setAdmTypeByAreaCode(data, "all");
			} else if (data.getArea_cd().length() == 2) {
				this.setAdmTypeByAreaCode(data, "sido");
			} else if (data.getArea_cd().length() == 5) {
				this.setAdmTypeByAreaCode(data, "sgg");
			} else if (data.getArea_cd().length() == 7) {
				this.setAdmTypeByAreaCode(data, "dong");
			}
		} else {
			throw new SystemFailException("선택된 area_cd가 없습니다");
		}
		
		return data;
	}
	
	private MyDataAnalysis setAdmTypeByAreaCode(MyDataAnalysis data, String type) {
		if (type.equals("all")) {
			data.setAdmType("all");
			data.setSido("");
		} else if (type.equals("sido")
			|| type.equals("sgg")
			|| type.equals("dong")) {
			data.setAdmType("sido");
			data.setSido(data.getArea_cd().substring(0, 2));
		}
		
		if (type.equals("sgg")
			|| type.equals("dong")) {
			data.setAdmType("sgg");
			data.setSgg(data.getArea_cd().substring(0, 5));
		}
		
		if (type.equals("dong")) {
			data.setAdmType("dong");
			data.setDong(data.getArea_cd().substring(0, 7));
		}
		
		return data;
	}
	
	private HashMap<String, Object> insertParametersByAdmType(
			MyDataAnalysis myDataAnalysis,
			HashMap<String, Object> parameters) {
		String admType;
		
		admType = myDataAnalysis.getAdmType();
		
		if ("all".equals(admType)) {
			// parent: 전국
			// children: 시도
			
			parameters = this.getAllParentParameters(parameters, myDataAnalysis.getSido());
		} else if ("sido".equals(admType)) {
			// parent: 시도
			// children: 시군구 
			
			parameters = this.getSidoParentParameters(parameters, myDataAnalysis.getSido());
			...
			if ("all".equals(myDataAnalysis.getSgg())) {
				...
				parameters = this.getSggParentParameters(parameters, myDataAnalysis.getSgg());
			}
			...
			if ("all".equals(myDataAnalysis.getDong())) {
				parameters = this.getSggParentParameters(parameters, myDataAnalysis.getSgg());
			} else {
				parameters = this.getDongParentParameters(parameters, myDataAnalysis.getDong());
			}
		}
	...
	private HashMap<String, Object> getAllParentParameters(
			HashMap<String, Object> parameters,
			String sido) {
		parameters.put("CURRENT_POLYGON_TABLE", "bnd_sido_pg");
		parameters.put("CURRENT_GEO_DATA_CD_COLUMN", "sido_cd");
		parameters.put("CHILDREN_POLYGON_TABLE", "bnd_sido_pg");
		parameters.put("CHILDREN_GEO_DATA_CD_COLUMN", "sido_cd");
		parameters.put("POLYGON_TABLE_KEY", "");
		parameters.put("ACTION_TYPE", "RANK_ANALY_ALL");
		
		return parameters;
	}
	
	private HashMap<String, Object> getSidoParentParameters(
			HashMap<String, Object> parameters,
			String sido) {
		parameters.put("CURRENT_POLYGON_TABLE", "bnd_sido_pg");
		parameters.put("CURRENT_GEO_DATA_CD_COLUMN", "sido_cd");
		parameters.put("CHILDREN_POLYGON_TABLE", "bnd_sigungu_pg");
		parameters.put("CHILDREN_GEO_DATA_CD_COLUMN", "sigungu_cd");
		parameters.put("POLYGON_TABLE_KEY", sido);
		parameters.put("ACTION_TYPE", "RANK_ANALY_SIDO");
		
		return parameters;
	}
	
	private HashMap<String, Object> getSggParentParameters(
			HashMap<String, Object> parameters,
			String sgg) {
		parameters.put("CURRENT_POLYGON_TABLE", "bnd_sigungu_pg");
		parameters.put("CURRENT_GEO_DATA_CD_COLUMN", "sigungu_cd");
		parameters.put("CHILDREN_POLYGON_TABLE", "bnd_adm_dong_pg");
		parameters.put("CHILDREN_GEO_DATA_CD_COLUMN", "adm_dr_cd");
		parameters.put("POLYGON_TABLE_KEY", sgg);
		parameters.put("ACTION_TYPE", "RANK_ANALY_SGG");
		
		return parameters;
	}
	
	private HashMap<String, Object> getDongParentParameters(
			HashMap<String, Object> parameters,
			String dong) {
		parameters.put("CURRENT_POLYGON_TABLE", "bnd_adm_dong_pg");
		parameters.put("CURRENT_GEO_DATA_CD_COLUMN", "adm_dr_cd");
		parameters.put("CHILDREN_POLYGON_TABLE", "bnd_total_oa_pg");
		parameters.put("CHILDREN_GEO_DATA_CD_COLUMN", "tot_oa_cd");
		parameters.put("POLYGON_TABLE_KEY", dong);
		parameters.put("ACTION_TYPE", "RANK_ANALY_DONG");
	...
	public String getBatchSpaData( JSONObject json, String userid) throws SystemFailException{
		...
		*/			
				...
				if( action_type != null && !"".equals(action_type) ){
					...
					//행정
					if( action_type.startsWith("ADMIN") ){
						String adm_cd = "adm_cd";
						//시도
